...
export const StudentDashboard: React.FC<StudentDashboardProps> = ({
  ...
  const refreshData = () => {
    ...
    if (s) {
      ...
      setActiveSessions(allActive.filter(sess => studentCourses.has(sess.courseId)));
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {activeSessions.length > 0 && (
        <div className="p-4.5 rounded-2xl bg-emerald-500/[0.04] border border-emerald-200/80 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-4 animate-fade-in relative overflow-hidden">
          ...
          <div className="flex items-start sm:items-center gap-3.5">
            ...
            <div className="text-left">
              ...
              <p className="text-xs text-slate-500 mt-1">
                Lecturer <span className="font-semibold text-slate-800">{activeSessions[0].lecturerName}</span> has opened attendance. Scan immediately.
              </p>
            </div>
          </div>
          <button
            id="join-active-session-scan-btn"
            onClick={() => onNavigate('scan')}
